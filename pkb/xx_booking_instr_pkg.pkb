DROP PACKAGE BODY APPS.XX_BOOKING_INSTR_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_BOOKING_INSTR_PKG" 
IS
----------------------------------------------------------------------
/*
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_daily_booking_instr_tmp;

      DELETE FROM xxintg.xx_daily_booking_instr_main;
      ...
      INSERT INTO xxintg.xx_daily_booking_instr_tmp
...
--               THEN NVL (team.email_address, NVL (sa.email_address,res.source_email))    	  --Commented By Ravi Vishnu on 02-DEC-14 to RESOURCE mail
			   THEN NVL (NVL (sa.email_address,res.source_email), team.email_address) 		  --Added By Ravi Vishnu on 02-DEC-14 to RESOURCE mail
 ...
 /*         case when NVL (team.full_name, NVL (restl.RESOURCE_NAME, papf.full_name)) in (SELECT meaning
            FROM fnd_lookup_values where lookup_type = 'INTG NEURO EXCEPTION LIST' and language = userenv('LANG') and enabled_flag = 'Y')
            then :P_ALT_SALES_REP ELSE NVL (team.email_address, NVL (sa.email_address,res.source_email)) END
                        SalesRep_Email_Alt,  */
...
--                   AND TRUNC (oha.booked_date) BETWEEN '04-MAY-2014' and TRUNC(SYSDATE)
                   ...
                   AND TRUNC (oha.booked_date) BETWEEN NVL (
                                                          TO_DATE (
                                                             p_from_date,
                                                             'DD-MON-RRRR'
                                                          ),
 ...
 --                  AND RES.RESOURCE_ID = NVL(:P_SALESREP, RES.RESOURCE_ID)
                  ...
                  AND (TO_NUMBER (DECODE (ola.line_category_code,'ORDER',ola.unit_selling_price * ola.ordered_quantity,'RETURN',-1 * ola.unit_selling_price *ola.ordered_quantity)) <> 0 )	--Added By Ravi Vishnu on 02-DEC-14 to exclude $0 Orders
											);
        ...
        IF l_database = 'PROD' THEN
		 ...
		 UPDATE xxintg.xx_daily_booking_instr_tmp x
        ...
        INSERT INTO xxintg.xx_daily_booking_instr_main
        SELECT * FROM xxintg.xx_daily_booking_instr_tmp x
                WHERE 1=1
