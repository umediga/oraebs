DROP PACKAGE BODY APPS.XXOM_CNSGN_MTL_XFER_IMP_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XXOM_CNSGN_MTL_XFER_IMP_PKG" 
/*************************************************************************************
*
*   HEADER
...
BEGIN
  INSERT INTO xxintg_cnsgn_cmn_imp_log_tbl 
...
BEGIN
      ...
      BEGIN
      fnd_global.apps_initialize (user_id => 1136, -- VISHY1634
         ...
         -- Selecting distribution account
              ...
              EXCEPTION
                 ...
                 /*
                      ...
                      */
                      log_message('Cannot find distribution account');
              END;
...
BEGIN
   ...
   IF p_salesrep_id in (-7,-14) THEN -- in ( -7, -14, -15) THEN
         l_secondary_inventory_name := 'KITFG';
         l_to_organization_code := '160';
         l_segment2 := substr(UPPER(l_division),1,3); -- UAT
         l_segment3 := 'ODC';
   ELSIF p_salesrep_id = -15 THEN 
         l_secondary_inventory_name := 'KITFG';
         l_to_organization_code := '180'; 
         l_segment2 := substr(UPPER(l_division),1,3); -- UIT
         l_segment3 := 'VDC';
   END IF; 
      ...
      log_message('l_division: '|| substr(UPPER(l_division),1,3));
       ...
       AND   mil.segment1 = ms.secondary_inventory_name
   ...
   ELSE
       ...
       FROM  hr_employees he,
             ...
             mtl_item_locations mil
       WHERE he.employee_id = p_salesrep_id
       AND   he.full_name = hl.location_code
       AND   hl.location_id = ms.location_id
       AND   ms.secondary_inventory_name = mil.subinventory_code
       AND   mil.segment1 = ms.secondary_inventory_name
       AND   mil.segment2 = 'REC'   -- assumption of setup per FS
       AND   mil.segment3 = '001';  -- assumption of setup per FS
...
WHEN NO_DATA_FOUND THEN
   p_return_message := 'Unable to find inventory location id for: ' || p_subinventory || ' and ' || p_locator;
   p_return_status := 'E';
WHEN OTHERS THEN
...
BEGIN
   ...
   BEGIN      
      ...
      log_message ('porganization_id: ' || p_organization_id);
   ...
   IF p_xfer_item IS NOT NULL THEN
     -- dbms_output.put_line ('getting item info...');
      ...
      INTO     l_inventory_item_id, l_primary_uom, 
             l_expiration_date, l_origination_date, l_source_lot_control_code, l_source_serial_control_code 
   ...
   log_message('x_mtl_txn_int.transaction_interface_id: ' || x_mtl_txn_int.transaction_interface_id);
   ...
   -- x_mtl_txn_int.transaction_reference := p_header_id;
   IF p_transaction_type_id = 21 THEN
      select wsh_deliveries_s.nextval
   ...
   IF (p_lot_number IS NOT NULL AND l_source_lot_control_code = 2 and p_transaction_type_id = 31) or 
        (p_lot_number IS NOT NULL AND l_dest_lot_control_code = 2 and p_transaction_type_id = 3) THEN
      BEGIN
        --dbms_output.put_line ('processing lot...for interface id: '||x_mtl_txn_int.transaction_interface_id);
        ...
        x_mtl_lot_int.transaction_quantity     := p_xfer_quantity;
   ...
   IF (p_serial_number IS NOT NULL AND l_source_serial_control_code in (2,5) and p_transaction_type_id = 31) OR 
        (p_serial_number IS NOT NULL AND l_dest_serial_control_code in (2,5) and p_transaction_type_id = 3) THEN
                  -- AND serial_number_control_code <> 1 
      -- dbms_output.put_line ('processing serial for interface id...'||x_mtl_txn_int.transaction_interface_id);
       BEGIN
         ...
         x_mtl_ser_int.fm_serial_number         := p_serial_number;
         x_mtl_ser_int.to_serial_number         := p_serial_number; 
...
IS
   ...
   CURSOR lpn_contents_cur (p_lpn_id number) IS
      ...
      SELECT wlpn.organization_id, wlpn.subinventory_code, wlpn.locator_id, wlpn.lpn_context, 
             wlpn.lpn_id, wlpn.license_plate_number, wlc.inventory_item_id, wlc.lot_number, 
             wlc.quantity, wlc.uom_code, msib.segment1 item, msib.lot_control_code, msib.serial_number_control_code
             --decode(msn.serial_number,wlpn.license_plate_number,msn.serial_number,null) serial_number
      FROM  wms_license_plate_numbers wlpn, wms_lpn_contents wlc, 
            mtl_system_items_b msib
      WHERE wlpn.license_plate_number = p_container
      AND   wlc.parent_lpn_id = wlpn.lpn_id
      AND   wlpn.organization_id = msib.organization_id
      AND   wlc.inventory_item_id = msib.inventory_item_id; 
      *****/
...
BEGIN
 ...
 SELECT  APPS.MTL_MATERIAL_TRANSACTIONS_S.NEXTVAL into l_set_info_batch_id FROM DUAL;
  -- l_transaction_items := xxintg_t_trx_line_t();
  ...
  -- l_transaction_source_name := 'INV CONV';
   ...
   -- from location
         log_message('from location: ' );
   get_inventory_location
        ( p_organization_id  => l_organization_id,
          p_salesrep_id  => p_salesrep_id,
          p_subinventory => l_subinventory,
          p_locator      => l_inventory_location_id,
          p_inventory_location_id => l_inventory_location_id,
          p_return_status =>  l_return_status,
          p_return_message =>  l_return_message
        );
        
      log_message('get_inventory_location from successful: ' || l_return_message);  
        log_message('To org after calling api ' || l_organization_id); 
        log_message('To org after calling api ' || l_subinventory); 
        log_message('To org after calling api ' || l_inventory_location_id); 
        
   IF ( l_return_status <> 'S' ) THEN
      log_message('l_return_message: ' || l_return_message);
   END IF;     
   
      log_message('to location: ' );
   -- to location
   get_inventory_location
        ( p_organization_id  => l_to_organization_id,
          p_salesrep_id  => p_salesrep_id,
          p_subinventory => l_to_subinventory,
          p_locator      => l_to_locator,
   ...
   END IF; 
     ...
     ******/ 
   l_subinventory := p_subinventory_code;
   l_to_subinventory := p_to_subinventory_code;
   l_inventory_location_id := p_locator_id;
   l_organization_id := p_organization_id;
     
    /****
    BEGIN
      SELECT milk.inventory_location_id, mp.organization_id
             INTO l_inventory_location_id, l_organization_id
             FROM  mtl_item_locations_kfv milk, mtl_parameters mp
             WHERE mp.organization_code = p_organization_code
             AND   milk.organization_id = mp.organization_id
             AND   milk.subinventory_code = l_subinventory
             AND   milk.concatenated_segments = l_locator; 
        EXCEPTION
      WHEN no_data_found THEN
         log_message('Cannot get locator ID for from info' || l_locator);
    END;
    ****/
      log_message('to org code ' || p_to_organization_code); 
      log_message('to sub code ' || l_to_subinventory); 
      log_message('to locator code ' || p_to_locator); 
     
    BEGIN
      SELECT inventory_location_id, mp.organization_id
             INTO l_to_inventory_location_id, l_to_organization_id
             FROM  mtl_item_locations_kfv milk, mtl_parameters mp
             WHERE mp.organization_code = p_to_organization_code
             AND   milk.organization_id = mp.organization_id
             AND   milk.subinventory_code = l_to_subinventory
             AND   milk.concatenated_segments = p_to_locator; 
        EXCEPTION
      WHEN no_data_found THEN
         log_message('Cannot get locator ID for to info' || p_to_locator);
    END;  
  ...
  -- l_transaction_items := p_tranaction_items;
   ...
   --l_transaction_type_id := 2; -- Subinventory Transfer
   l_source_code :=  'Inventory'; -- 'Inter Org';
   
    
   
   log_message('from org id: ' || l_organization_id);
   log_message('from sub code: ' || l_subinventory);
   log_message('from locator id: ' || l_inventory_location_id);
   log_message('p_container: ' || p_container);
   log_message('to org id ' || l_to_organization_id); 
   log_message('to sub code ' || l_to_subinventory); 
   log_message('to locator id ' || l_to_inventory_location_id); 
   ...
   IF ( p_container IS NOT NULL ) THEN
      ...
      LOOP
      log_message('In loop: ' || serial_rec.lpn_id);
      ...
      loop
        ...
        begin
        select snm_division into l_division from XXOM_SALES_MARKETING_SET_V where inventory_item_id = lpn_contents_rec.inventory_item_id
          and organization_id = lpn_contents_rec.organization_id;
        exception
          when no_data_found then
            log_message('Did not find division for item: ' || lpn_contents_rec.inventory_item_id);
        end;
      ...
      log_message('j: ' || j || ' item: ' || lpn_contents_rec.item || ' quantity: ' || lpn_contents_rec.quantity || 'lpn_contents_rec.serial_number: '||lpn_contents_rec.serial_number);
         ...
         IF ( lpn_contents_rec.serial_number = p_container ) 
          ...
          -- l_transaction_type_id := 21; -- Intransit Shipment
           l_source_code :=  'Onhand Inter Org';
          ...
          consignment_transaction
            (
              ...
              IF (l_return_status <> fnd_api.g_ret_sts_success) THEN
                log_message(' Main kit not transferred --> ' || lpn_contents_rec.serial_number);
         ...
         ELSE
            ...
            l_source_code :=  'Onhand Alias Issue';
            ...
            (
             ...
             /****Insert into set table for contents ***/
         ...
         END IF; -- If top level or components    
   ...
   IF l_return_status <> fnd_api.g_ret_sts_success THEN
      IF x_msg_count > 0 THEN
        ...
        LOOP
            ...
            log_message('x_return_message: '|| x_return_message);
        END LOOP;
      END IF;
   END IF;
...
END XXOM_CNSGN_MTL_XFER_IMP_PKG;
/
