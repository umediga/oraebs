DROP PACKAGE BODY APPS.XX_DAILY_SALES_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_DAILY_SALES_PKG" 
IS
----------------------------------------------------------------------
/*
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_daily_inv_recon_tmp;

      DELETE FROM xxintg.xx_daily_inv_recon_main;
      ...
      INSERT INTO xxintg.xx_daily_inv_recon_tmp
      (SELECT   DISTINCT NVL (team.team_resource_id, res.resource_id) resource_id,UPPER(caa.customer_class_code) Customer_type,
         ...
         * DECODE (ctla.quantity_invoiced,
            ...
            total_sales_dollars,
            NULL   mgr_name,
         sa.RESOURCE_ID mgr_email,
         NVL (team.full_name, NVL (restl.RESOURCE_NAME, papf.full_name))
            SalesRep_Name,
		 CASE
            WHEN ( (SELECT   name FROM v$database) = 'PROD')
            THEN
               NVL (team.email_address,
                    NVL (sa.email_address, res.source_email))
            ELSE
...
--                                               )
         ...
         UPDATE xxintg.xx_daily_inv_recon_tmp x
         set order_date = (select trunc(ordered_date) from oe_order_headers_all where order_number = x.order_number),
--		     booked_date = (select trunc(booked_date) from oe_order_headers_all where order_number = x.order_number),   -- Commented By Ravi Vishnu on 24-DEC-2014 to handle NULL Booked Date Orders
             booked_date = (select NVL(trunc(booked_date), trunc(ordered_date)) from oe_order_headers_all where order_number = x.order_number),  -- Added By Ravi Vishnu on 24-DEC-2014 to handle NULL Booked Date Orders
        ...
        INSERT INTO xxintg.xx_daily_inv_recon_main
        SELECT * FROM xxintg.xx_daily_inv_recon_tmp x
                WHERE 1=1
		          and resource_id=to_number(mgr_email)
