DROP PACKAGE BODY APPS.XX_BACKORDER_INSTR_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_BACKORDER_INSTR_PKG" 
IS
----------------------------------------------------------------------
/*
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_backorder_instr_tmp;

      DELETE FROM xxintg.xx_backorder_instr_main;
      ...
      INSERT INTO xxintg.xx_backorder_instr_tmp
...
--         DD.RELEASED_STATUS Line_Status,
        ...
        --       THEN NVL (team.email_address, NVL (sa.email_address,res.source_email))		-- Commented By Ravi Vishnu for Resource Email Logic
		       THEN NVL (NVL (sa.email_address,res.source_email), team.email_address )		-- Added By Ravi Vishnu for Resource Email Logic
...
--         AND OHA.ORDER_NUMBER = '114631' --'117690'
         ...
         AND upper(ott.name) in ('ILS BILL ONLY ORDER','ILS CHARGE SHEET ORDER','ILS STANDARD ORDER','ILS RETURN ORDER'--,'ILS SAMPLE ORDER'   --Commented By Ravi Vishnu on 08-AUG-14 to exclude Sample Orders
...
--         AND RES.RESOURCE_ID = NVL(:P_SALESREP, RES.RESOURCE_ID)
        ...
        AND (TO_NUMBER (DECODE (OLA.LINE_CATEGORY_CODE,'ORDER',OLA.UNIT_SELLING_PRICE * OLA.ORDERED_QUANTITY,'RETURN',-1 * OLA.UNIT_SELLING_PRICE * OLA.ORDERED_QUANTITY)) <> 0 ) --Added By Ravi Vishnu to exclude $0 Orders on 02-DEC-2014
         );
        ...
        IF l_database = 'PROD' THEN
		 ...
		 UPDATE xxintg.xx_backorder_instr_tmp x
        ...
        INSERT INTO xxintg.xx_backorder_instr_main
        SELECT * FROM xxintg.xx_backorder_instr_tmp x
                WHERE 1=1
