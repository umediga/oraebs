DROP PACKAGE BODY APPS.XX_DAILY_SALES_INSTR_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_DAILY_SALES_INSTR_PKG" 
IS
----------------------------------------------------------------------
/*
 ...
 Description   : This code is being written to get data  for Instr Daily sales
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_daily_inv_instr_tmp;

      DELETE FROM xxintg.xx_daily_inv_instr_main;
      ...
      INSERT INTO xxintg.xx_daily_inv_instr_tmp
      (SELECT  DISTINCT NVL (team.team_resource_id, res.resource_id) resource_id,UPPER(caa.customer_class_code) Customer_type,
		 ...
		 (select name from oe_transaction_types_tl a, oe_order_headers_all b
            where a.transaction_type_id=b.order_type_id
            and a.language='US'
            and  order_number=cta.interface_header_attribute1) order_type,
         (SELECT ORDERED_DATE FROM oe_order_headers_all WHERE order_number=cta.interface_header_attribute1)  Order_date,
		 (SELECT BOOKED_DATE FROM oe_order_headers_all WHERE order_number=cta.interface_header_attribute1)  Booked_date,
...
--         icv.category_set_name sales_group_cat_set,
         ...
         ctla.unit_selling_price * DECODE (ctla.quantity_invoiced,NULL, ctla.quantity_credited,ctla.quantity_invoiced) total_sales_dollars,
         NULL mgr_name,
         NULL mgr_email,
         --                       NVL(team.full_name,NVL (res.source_email, papf.email_address)) Team_SalesRep_Name,
         --                       NVL(team.email_address, nvl(res.source_name,papf.full_name))  Team_Email
         NVL (team.full_name, NVL (restl.RESOURCE_NAME, papf.full_name)) SalesRep_Name,
        CASE WHEN ((SELECT name FROM v$database) = 'PROD')
--               THEN NVL (team.email_address, NVL (sa.email_address,res.source_email))	   -- Commented By Ravi Vishnu for the Resource Mail Logic
			   THEN NVL (NVL (sa.email_address,res.source_email), team.email_address)	   -- Added By Ravi Vishnu for the Resource Mail Logic
		...
		jrb.manager_flag MANAGER_FLAG,
         (SELECT   osc.attribute1
           ...
           WHERE       1 = 1
                   AND osc.header_id = ooh.header_id
                   AND osc.line_id = ool.line_id
                   AND ooh.header_id = ool.header_id
                   AND ooh.org_id = ool.org_id
                   AND ooh.order_number = cta.interface_header_attribute1
                   AND ool.line_id = ctla.interface_line_attribute6
   ...
   WHERE       cta.customer_trx_id = ctla.customer_trx_id
           ...
           AND cta.org_id = ctype.org_id ------ Commented by balaji
           AND cta.ship_to_customer_id = caa.cust_account_id
           AND SA.salesrep_id = ctlsa.salesrep_id--cta.primary_salesrep_id -- By Balaji
...
--                 (SELECT   1
--                    FROM   APPS.oe_sales_credits OSC
--                   WHERE   1 = 1 AND osc.salesrep_id(+) = ctlsa.salesrep_id--cta.primary_salesrep_id
--                   ) --- Commented by balaji
          ...
          AND res.resource_id = restl.resource_id(+)
           AND team.trsid(+)=sa.RESOURCE_ID--RES.resource_id    --- Commented by balaji
--           AND UPPER (mc.segment4) = 'INSTR' -- 'NEURO' -- 'RECON'
--           and cta.trx_number ='23051261'
  ...
  /*         AND (cta.trx_date) BETWEEN NVL (TO_DATE (p_from_date, 'DD-MON-RRRR'),
                                           (cta.trx_date))
                                  AND  NVL (TO_DATE (p_to_date, 'DD-MON-RRRR'),
                                            (cta.trx_date)) */
		   AND (cta.trx_date) BETWEEN NVL (TO_DATE(p_from_date,'DD-MON-RRRR'),(cta.trx_date)) AND NVL (TO_DATE(p_to_date,'DD-MON-RRRR'), (cta.trx_date))
--           AND UPPER (mc.segment9) = UPPER (NVL (:D_Code, mc.segment9))
...
--		   AND jrr.role_resource_id = NVL (team.trsid, NVL (restl.resource_id, papf.person_id)) --100022102  -- Commented By Ravi Vishnu
           AND jrr.role_resource_id = NVL (team.team_resource_id, NVL (restl.resource_id, papf.person_id))  -- Added By Ravi Vishnu
		   AND jrb.role_type_code like 'SALES_COMP'
		   AND jrb.manager_flag <> 'Y'  --Exclude Manager
           AND UPPER(jrbt.role_name) NOT LIKE '%SPINE%'
		   AND ( ctla.unit_selling_price * DECODE (ctla.quantity_invoiced,NULL, ctla.quantity_credited,ctla.quantity_invoiced) ) <> 0
         );

         COMMIT;

         UPDATE xxintg.xx_daily_inv_instr_tmp x
         set order_date = (select trunc(ordered_date) from oe_order_headers_all where order_number = x.order_number),
             booked_date = (select NVL(trunc(booked_date), trunc(ordered_date)) from oe_order_headers_all where order_number = x.order_number),
        ...
        INSERT INTO xxintg.xx_daily_inv_instr_main
        SELECT * FROM xxintg.xx_daily_inv_instr_tmp x
                WHERE 1=1
