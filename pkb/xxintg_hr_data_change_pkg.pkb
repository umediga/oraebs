DROP PACKAGE BODY APPS.XXINTG_HR_DATA_CHANGE_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XXINTG_HR_DATA_CHANGE_PKG" 
AS
----------------------------------------------------------------------
/*
...
IS
    BEGIN
        INSERT INTO xxintg_hr_data_chgs_hist
           SELECT * from xxintg_hr_data_changes
           where creation_date < p_date;

         DELETE FROM xxintg_hr_data_changes
         where creation_date < p_date;
   END  archive_table;
...
***************************************************************************/
   ...
   IS
   BEGIN
      INSERT INTO xxintg_hr_data_changes
                  (request_id, person_id, employee_number,
                   full_name, position_id, position_name,
                   organization_id, organization_name,location_id,location_name,job_id,job_name, cost_string,
           ...
           VALUES (p_request_id, p_person_id, p_employee_number,
                   ...
                   p_organization_id, p_organization_name,p_location_id,p_location_name,p_job_id,p_job_name,p_cost_string,
   ...
   IS
      ...
      IS
         SELECT papf.person_id, papf.employee_number, papf.full_name,
                ...
                haou.organization_id, haou.NAME org_name,hla.location_id,hla.location_code location_name
                ,pj.job_id,pj.name job_name,
      ...
      IS
         SELECT *
           FROM xxintg_hr_data_changes
          WHERE request_id = g_request_id;

      CURSOR csr_previous_run_data (p_person_id NUMBER)
      IS
         SELECT DISTINCT person_id, organization_id, position_id,
                         supervisor_id, supervisor_name, organization_name,job_id,job_name,location_id,location_name,
    ...
    SELECT parameter_value email_id
     ...
     WHERE ps.process_id = pp.process_id
       and PS.PROCESS_NAME = 'XXHRJOBTITLECHGS'
       AND pp.parameter_name = 'Email_Address';
   ...
   BEGIN
      ...
      LOOP
         ...
         insert_staging_data (g_request_id,
                              ...
                              rec_emp_data.location_id,
                              rec_emp_data.location_name,
                              rec_emp_data.job_id,
                              rec_emp_data.job_name,
     ...
     /*    l_field_chg_data_head :=
         ...
         LOOP
            ...
            LOOP
               ...
               THEN
                  l_changed := 'Y';
               ELSIF nvl(rec_previous_run_data.location_id,'XXXX')!=
                                                 nvl(rec_latest_data.location_id,'XXXX')
               THEN
                  l_changed := 'Y';
               ELSIF nvl(rec_previous_run_data.job_id,'XXXX')!=
                                                 nvl(rec_latest_data.job_id,'XXXX')
               THEN
                  l_changed := 'Y';
               ELSE
                  l_changed := 'N';
          ...
          PUT_LINE (
            FND_FILE.OUTPUT,
            '<PRE_JOB>' || replace(rec_previous_run_data.job_name, '&', '&'||'amp;') || '</PRE_JOB>');
            FND_FILE.
          PUT_LINE (
            FND_FILE.OUTPUT,
            '<JOB_NAME>' || replace(rec_latest_data.job_name, '&', '&'||'amp;') || '</JOB_NAME>');
            FND_FILE.
          PUT_LINE (
            FND_FILE.OUTPUT,
            '<PRE_LOCATION>' || replace(rec_previous_run_data.location_name, '&', '&'||'amp;') || '</PRE_LOCATION>');
            FND_FILE.
          PUT_LINE (
            FND_FILE.OUTPUT,
            '<LOCATION_NAME>' || replace(rec_latest_data.location_name, '&', '&'||'amp;') || '</LOCATION_NAME>');
           FND_FILE.
          PUT_LINE (
            FND_FILE.OUTPUT,
          ...
          FND_FILE.PUT_LINE (FND_FILE.OUTPUT, '</HRNOTF_REPORT>');
            ...
            IF (rec_latest_data.change_type ='NEW HIRE') THEN
       ...
       FND_FILE.PUT_LINE (FND_FILE.OUTPUT, '<OLM_ADMIN_EMAIL>');

       FND_FILE.
          PUT_LINE (
            FND_FILE.OUTPUT,
            '<OLM_EMAIL>' || x_email_recipients || '</OLM_EMAIL>');
