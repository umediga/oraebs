DROP PACKAGE BODY APPS.XX_BACKORDER_INSTR_RM_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_BACKORDER_INSTR_RM_PKG" 
IS
----------------------------------------------------------------------
/*
 Created By    : Ravi Vishnu
 Creation Date : 05-Nov-2014
 File Name     : xx_backorder_instr_rm_pkg
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_backorder_instr_rm_tmp;

      DELETE FROM xxintg.xx_backorder_instr_rm_main;
      ...
      INSERT INTO xxintg.xx_backorder_instr_rm_tmp
...
--         AND OHA.ORDER_NUMBER = '114631' --'117690'
  ...
  --        AND TRUNC (oha.booked_date)  between '01-OCT-2014' AND '10-OCT-2014'
...
--         AND RES.RESOURCE_ID = NVL(:P_SALESREP, RES.RESOURCE_ID)
	...
	--	 AND jrb.manager_flag <> 'Y'  --Exclude Manager
        ...
        IF l_database = 'PROD' THEN
		 ...
		 UPDATE xxintg.xx_backorder_instr_rm_tmp x
        ...
        INSERT INTO xxintg.xx_backorder_instr_rm_main
        SELECT * FROM xxintg.xx_backorder_instr_rm_tmp x
                WHERE 1=1
