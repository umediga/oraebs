DROP PACKAGE BODY APPS.XX_DAILY_SALES_RM_PKG;

CREATE OR REPLACE PACKAGE BODY APPS."XX_DAILY_SALES_RM_PKG" 
IS
----------------------------------------------------------------------
/*
...
-------------------------------------
      ...
      DELETE FROM xxintg.xx_daily_inv_recon_rm_tmp;

      DELETE FROM xxintg.xx_daily_inv_recon_rm_main;
...
--      alter table xx_daily_invoice_recon2 modify (order_type varchar2(50))
--      alter table xx_daily_invoice_recon2 modify (mgr_name varchar2(200))
--      alter table xx_daily_invoice_recon2 modify (mgr_email varchar2(200))


      INSERT INTO xxintg.xx_daily_inv_recon_rm_tmp
...
--                        AND NVL(papf.current_employee_flag, 'N') = 'Y'			--Added By Ravi Vishnu to fetch only current Employee Details
...
--         AND jrb.manager_flag <> 'Y'  -- Commented to include Managers too
...
--         AND NVL(papf.current_employee_flag, 'N') = 'Y'			--Added By Ravi Vishnu to fetch only current Employee Details
         ...
         UPDATE xxintg.xx_daily_inv_recon_rm_tmp x
        ...
        INSERT INTO xxintg.xx_daily_inv_recon_rm_main
        SELECT * FROM xxintg.xx_daily_inv_recon_rm_tmp x
                WHERE 1=1
