DROP VIEW APPS.XX_XRTX_AR_CR_TXN_SRC;

/* Formatted on 6/6/2016 4:57:07 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_XRTX_AR_CR_TXN_SRC
(
...
AS
   ...
   SELECT 'MANUAL' CONCURRENT_PROGRAM_NAME,
    ...
    WHERE b.program_id IS NULL AND B.STATUS IN ('UNID', 'UNAPP', 'APP')
   UNION ALL
   SELECT 'MANUAL' CONCURRENT_PROGRAM_NAME,
          TO_CHAR (TRUNC (b.creation_date), 'YYYY') YEAR,
          'MANUAL' creation_type,
          ...
          CASE WHEN EXTRACT (MONTH FROM b.creation_date) = 12 THEN 1 END L --"DEC",
     FROM AR_CASH_RECEIPTS_ALL b
    WHERE b.program_id IS NULL AND B.STATUS IN ('UNID')
   UNION ALL
   SELECT 'MANUAL' CONCURRENT_PROGRAM_NAME,
          TO_CHAR (TRUNC (b.creation_date), 'YYYY') YEAR,
          'MANUAL' creation_type,
          ...
          CASE WHEN EXTRACT (MONTH FROM b.creation_date) = 12 THEN 1 END L --"DEC",
     FROM AR_CASH_RECEIPTS_ALL b
    WHERE b.program_id IS NULL AND B.STATUS IN ('UNAPP')
   UNION ALL
   SELECT 'MANUAL' CONCURRENT_PROGRAM_NAME,
          TO_CHAR (TRUNC (b.creation_date), 'YYYY') YEAR,
          'MANUAL' creation_type,
          ...
          CASE WHEN EXTRACT (MONTH FROM b.creation_date) = 12 THEN 1 END L --"DEC",
     FROM AR_CASH_RECEIPTS_ALL b
    WHERE b.program_id IS NULL AND B.STATUS IN ('APP')
   ORDER BY 1,
            3,
            4,
            2 DESC;
