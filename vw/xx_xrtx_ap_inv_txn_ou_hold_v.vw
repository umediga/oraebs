DROP VIEW APPS.XX_XRTX_AP_INV_TXN_OU_HOLD_V;

/* Formatted on 6/6/2016 4:57:44 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_XRTX_AP_INV_TXN_OU_HOLD_V
(
   OPERATING_UNIT,
   YEAR,
   HOLD_LOOKUP_CODE,
...
AS
   SELECT c.name operating_unit,
     ...
     FROM ap_invoices_all A, ap_holds_all b, hr_all_organization_units c
    WHERE A.invoice_id = b.invoice_id AND A.org_id = c.organization_id;
