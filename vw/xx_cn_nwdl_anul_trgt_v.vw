DROP VIEW APPS.XX_CN_NWDL_ANUL_TRGT_V;

/* Formatted on 6/6/2016 4:58:49 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_CN_NWDL_ANUL_TRGT_V
(
   SALESREP_ID,
   LAST_PERIOD_ID,
   ORG_ID,
   NEWDEAL_ANNUAL_TARGET
)
AS
   ...
   GROUP BY period_quotas.salesrep_id,
            period_quotas.org_id,
            CEIL (
               (period_quotas.period_id - (ipv.start_period_id - 1)) / 12);
