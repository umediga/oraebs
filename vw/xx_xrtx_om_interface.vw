DROP VIEW APPS.XX_XRTX_OM_INTERFACE;

/* Formatted on 6/6/2016 4:56:39 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_XRTX_OM_INTERFACE
(
   ORG_ID,
   OPERATING_UNIT,
   SOURCE_NAME,
...
AS
   SELECT A.org_id AS org_id,
    ...
    WHERE     A.org_id = b.organization_id
          ...
          AND (   c.AIA_ENABLED_FLAG = 'Y'
               OR c.NAME IN ('EDI', 'XML', 'Custom Order Entry')
               OR UPPER (c.description) LIKE '%IMPORT%');
