DROP VIEW APPS.XX_ADI_MULTIPO_UPD_V;

/* Formatted on 6/6/2016 5:00:01 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_ADI_MULTIPO_UPD_V
(
   ACTION,
   PO_NUMBER,
   REL_NUM,
...
AS
     SELECT "ACTION",
       ...
       FROM (SELECT 'SYNC' action,
                    ...
                    TO_CHAR (plla.need_by_date, 'YYYY-MM-DD HH24:MI:SS')
                       need_by_date,
                    TO_CHAR (plla.promised_date, 'YYYY-MM-DD HH24:MI:SS')
                       promised_date,
                    aps.vendor_name,
                    pdt.type_name,
                    pha.segment1 po_rel_combination
             ...
             SELECT 'SYNC' action,
                    ...
                    aps.vendor_name,
                    pdt.type_name,
                    pha.segment1 || '-' || pr.release_num po_rel_combination
               FROM po_headers_all pha,
                    po_lines_all pla,
                    po_line_locations_all plla,
                    ap_suppliers aps,
