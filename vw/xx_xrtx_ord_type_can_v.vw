DROP VIEW APPS.XX_XRTX_ORD_TYPE_CAN_V;

/* Formatted on 6/6/2016 4:56:34 PM (QP5 v5.277) */
CREATE OR REPLACE FORCE VIEW APPS.XX_XRTX_ORD_TYPE_CAN_V
(
   NAME,
   CANCELLED_FLAG,
   YEAR,
...
AS
     SELECT b.name,
      ...
      WHERE     oha.ordered_date BETWEEN TO_DATE (
                                            CONCAT (
                                               '01-JAN-',
            ...
            AND b.language = 'US'
   GROUP BY b.name, oha.CANCELLED_FLAG, oha.ordered_date
   ORDER BY 2 DESC;
